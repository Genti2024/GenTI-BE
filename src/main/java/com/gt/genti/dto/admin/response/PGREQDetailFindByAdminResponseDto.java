...
public class PGREQDetailFindByAdminResponseDto {
	...
	@Schema(description = "프롬프트", example = "대통령처럼 많은 군중들 앞에서 양 팔을 들고 연설하고 있는 모습")
	String prompt;
	@Schema(description = "chatgpt api 결과 프롬프트", example = "Standing before a large crowd like a president, addressing them with both arms raised in a speech, (presidential demeanor:1.3), (addressing crowd:1.4), (arms raised:1.3), (public speaking:1.3), (commanding presence:1.3), (crowd engagement:1.2), (authoritative stance:1.3), (political speech:1.2), (captivating audience:1.3), (leader-like charisma:1.2)")
	String promptAdvanced;
	@Schema(description = "사용자의 얼굴 사진")
	List<CommonPictureResponseDto> facePictureList;
	@Schema(description = "포즈 참고 사진", nullable = true)
	...
	@Schema(description = "사진생성요청의 상태", example = "IN_PROGRESS")
	...
	public PGREQDetailFindByAdminResponseDto(PictureGenerateRequest pgreq) {
		...
		if (!pgreq.getResponseList().isEmpty()) {
			...
			Duration duration = Duration.between(LocalDateTime.now(),
				pgreq.getCreatedAt().plusHours(DateTimeUtils.PGREQ_LIMIT_HOUR));
			if (!duration.isNegative()) {
				this.remainTime = DateTimeUtils.getTimeString(duration);
			} else {
				this.remainTime = DateTimeUtils.getZeroTime();
			}
		}
	}
	...
	public PGREQDetailFindByAdminResponseDto(Long pictureGenerateRequestId, Long requesterId, String requesterEmail,
		...
		if (createdAt != null) {
			...
			Duration duration = Duration.between(LocalDateTime.now(),
				this.createdAt.plusHours(DateTimeUtils.PGREQ_LIMIT_HOUR));
			if (!duration.isNegative()) {
				this.remainTime = DateTimeUtils.getTimeString(duration);
			} else {
				this.remainTime = DateTimeUtils.getZeroTime();
