...
public class UserPGREQController {
	...
	@Operation(summary = "진행중인 사진생성요청 유무 조회", description = "작업이 진행중인 사진생성요청이 있는지 여부를 조회한다.")
	@EnumResponses(value = {
		@EnumResponse(ResponseCode.OK)
	})
	@GetMapping("/pending")
	...
	public ResponseEntity<ApiResult<PGREQBriefFindByUserResponseDto>> findNotVerifiedCompletedPGREQ(
		...
		return success(
			pictureGenerateRequestUseCase.findByRequestAndStatusIs(userDetails.getUser(),
